...
# Per-GPU torch.compile (inductor) + Triton kernel cache root. Each physical
# GPU gets its own subdir (inductor codegen is arch-specific). Override with
# UNDERFIT_TORCH_COMPILE_CACHE_DIR to point it at fast local storage.
TORCH_COMPILE_CACHE_DIR = Path(os.environ.get(
    "UNDERFIT_TORCH_COMPILE_CACHE_DIR", STATE_DIR / "torch_compile_cache"
)).expanduser()
...
def estimate_training_vram_mb(base_model="sa3", batch_size=8, lora_rank=16, precision="16-mixed"):
    ...
    - Base: fp32=11,500 MB, fp16/bf16=6,670 MB (weights + CUDA context + gradient buffers)
    ...
    if "bf16" in p:
        base_mb = 6670  # bf16 weights = 2 bytes, same as fp16
    elif "16" in p or "mixed" in p:
        base_mb = 6670  # fp16 weights
    else:
        base_mb = 11500  # fp32 weights
...
def _backend_env_for_model(model_key):
    """Return an env-var dict to set UNDERFIT_BACKEND for a given base model.
...
def _torch_compile_cache_env(cuda_visible):
    """Return per-GPU torch.compile cache env vars for a CUDA_VISIBLE_DEVICES value.

    Inductor codegen is arch-specific (sm_86 vs sm_120), so each physical GPU
    gets its own cache dir keyed by CUDA_VISIBLE_DEVICES. Returns {} when no GPU
    is pinned (e.g. a CPU-only launch). Both the inductor codegen cache and the
    Triton cubin cache are pointed here so repeated runs load, not recompile.
    """
    if not cuda_visible:
        return {}
    key = "gpu" + str(cuda_visible).strip().replace(",", "-")
    root = str(TORCH_COMPILE_CACHE_DIR / key)
    os.makedirs(root, exist_ok=True)
    os.makedirs(os.path.join(root, "triton"), exist_ok=True)
    return {
...
def _build_launch_command(script_path, script_args, env_vars, cwd=None):
    """Build cross-platform subprocess args list + env dict.

    On Linux: returns (["bash", "-c", "source activate && python ..."], env)
    On Windows: returns ([VENV_PYTHON, script, ...args...], env)
    ...
    script_args : list[str]
        Arguments to pass to the script (not including the script name itself).
    env_vars : dict[str, str | None]
        Environment variables to set. Keys with None values are skipped.
    ...
    -------
    (args_list, env_dict)
        args_list: list of strings for subprocess.Popen
        env_dict: dict of environment variables (merged with os.environ)
    """
    # Build environment (merge with parent env, skip None values)
    launch_env = os.environ.copy()
    for k, v in env_vars.items():
        if v is not None:
            launch_env[k] = str(v)

    # Persistent per-GPU torch.compile (inductor) + Triton kernel cache. Keyed
    # by physical GPU (CUDA_VISIBLE_DEVICES) because inductor codegen is
    # arch-specific (sm_86 vs sm_120). Repeated/queued runs and the gradio
    # inference server reuse compiled kernels instead of recompiling.
    launch_env.update(_torch_compile_cache_env(launch_env.get("CUDA_VISIBLE_DEVICES")))

    if IS_WINDOWS:
        # Direct python invocation — no shell needed
        args = [str(VENV_PYTHON), str(script_path)] + [str(a) for a in script_args]
        return args, launch_env, cwd
    else:
        # Linux: bash wrapper to source venv activation
        # Build inline env prefix: VAR1=val1 VAR2=val2 ...
        inline_env_parts = []
        for k, v in env_vars.items():
            if v is not None:
                inline_env_parts.append(f"{k}={shlex.quote(str(v))}")
        inline_env = " ".join(inline_env_parts)

        # NOTE: do NOT filter out empty-string args. Flags like
        # --val-dataset-config / --pretransform-ckpt-path / --ckpt-path /
        # --logger are known value-taking flags, so an empty value must still
        # be passed (as '') or argparse consumes the next flag as its value.
        cmd_parts = [str(VENV_PYTHON), str(script_path)] + [shlex.quote(a) for a in script_args]
        cmd_str = " ".join(cmd_parts)

        bash_cmd = f"source {shlex.quote(VENV_ACTIVATE)} && "
        if cwd:
            bash_cmd += f"cd {shlex.quote(str(cwd))} && "
        if inline_env:
            bash_cmd += f"{inline_env} "
        bash_cmd += cmd_str

        return ["bash", "-c", bash_cmd], launch_env, cwd


def _split_restart_cmd(cmd):
    """Split a stored restart_cmd into (script_path, script_args).

    Tolerates a leading python interpreter token (``python`` / ``python3`` /
    ``python.exe`` / any path to one) and expands leading-``~`` home paths so
    the recovered paths are absolute and usable on both platforms. The first
    remaining token is the script (e.g. lora_train.py); the rest are its args.
    ...
    if not parsed:
        return None, []
    # Drop a leading interpreter token if present (any path ending in
    # python / python3(.exe)). _build_launch_command re-prepends VENV_PYTHON.
    base = os.path.basename(parsed[0]).lower().split(".")[0]
    ...
    def _expand(tok):
        # Resolve ~ / ~/... to an absolute path (works regardless of the
        # slash style stored in restart_cmd).
        if tok == "~":
            return home
        if tok.startswith("~/") or tok.startswith("~\\"):
            return str(Path(home) / tok[2:])
        return tok
    parsed = [_expand(t) for t in parsed]
...
def _get_children_pids(pid):
    ...
    Uses psutil on both platforms for cross-platform compatibility.
    Falls back to empty list if psutil is unavailable or process is gone.
    """
    try:
        import psutil
...
def _get_process_cmdline(pid):
    """Return the command line string for a given PID.

    Uses psutil on both platforms. Falls back to empty string.
    """
    try:
        import psutil
        proc = psutil.Process(pid)
...
def _get_process_username(pid):
    """Return the username that owns the given PID.
...
def _free_gpu_memory(gpu):
    ...
    if gpu is None:
        return
    try:
        cmd_str = 'import torch; torch.cuda.empty_cache(); print("VRAM freed on GPU", ' + str(gpu) + ')'
        args = [str(VENV_PYTHON), "-c", cmd_str]
        env = os.environ.copy()
...
def _diagnose_quick_kill(run: dict) -> str | None:
    ...
    Checks in priority order:
      ...
      2. No `.started` marker — process never reached python.
    ...
    # 2. Did lora_train.py even reach __main__?
    started_path = log_path + ".started"
    reached_python = os.path.exists(started_path)
    if log_path and not reached_python:
        # No marker means process died before python ran. Most likely: bad venv
        # path, missing executable, or environment issue.
        return ("Process never reached python (no '.started' marker) — "
                "venv / environment issue. Try the run's restart_cmd manually.")
    ...
    if elapsed is not None and elapsed < 30 and log_size < 200:
        if IS_WINDOWS:
            return ("Process died in <30 s with no output — likely killed for "
                    "out-of-memory. Check Windows Event Viewer (System log) or "
                    "nvidia-smi for VRAM, or try a smaller batch size / model.")
...
def _save_tag_cache(ds_id, file_info, total_files, files_with_tags, files_with_json, files_with_txt=0):
...
def _load_tag_cache(ds_id):
    """Load cached tag data. Returns (file_info, total, with_tags, with_json, with_txt) or None."""
...
def _find_txt_sidecar(fpath, txt_map=None):
    """Find .txt sidecar for an audio file.

    Search order:
      1. Same directory: track.txt next to track.wav
      2. Sibling txt/ directory: parent/txt/track.txt
      3. Cross-directory: use txt_map to find matching stems.
    Returns the Path to the sidecar, or None.
    """
    fp = Path(fpath)
    # 1. Same directory (fast path)
    same_dir = fp.with_suffix(".txt")
    if same_dir.exists():
        return same_dir
    # 2. Sibling txt/ directory
    txt_dir = fp.parent / "txt" / (fp.stem + ".txt")
    if txt_dir.exists():
        return txt_dir
    # 3. Cross-directory lookup via pre-built map
    if txt_map:
        stem = fp.stem
        candidates = txt_map.get(stem)
        if candidates:
            if len(candidates) == 1:
                return candidates[0]
            audio_parts = fp.parent.parts
            ...
            for tpath in candidates:
                tp = tpath.parent.parts
                shared = 0
                for a, b in zip(audio_parts, tp):
                    if a == b:
                        shared += 1
                    else:
                        break
...
def _read_audio_tags(fpath, json_map=None, txt_map=None):
    """Read metadata tags from an audio file.

    Priority: JSON sidecar ({stem}.json) > .txt sidecar ({stem}.txt) > embedded tags.
    ...
    # Check for .txt sidecar (same dir, sibling txt/, or cross-directory)
    txt_sidecar = _find_txt_sidecar(fpath, txt_map)
    if txt_sidecar:
        try:
            content = txt_sidecar.read_text(encoding="utf-8", errors="replace").strip()
            if content:
...
class GradioManager:
    ...
    def launch(self, checkpoint_path, gpu, run_id=None, checkpoint_name=None, title=None, model_variant=None, verbose=False):
        with self._lock:
                ...
                for cfg_name in [f"{run_id}_model_resume.json", f"{run_id}_model.json"]:
                    ...
                    if cfg_p.exists():
                        try:
                            ...
                            if prompts:
                                default_prompt_arg = ["--default-prompt", random.choice(prompts)]
                        except Exception:
                            pass
                        break

            # Build gradio script args as a proper list (cross-platform)
            gradio_script_args = [
                "--model-config", config_path_model,
                "--ckpt-path", ckpt_path_model,
            ]
            if arc_lora_path:
                gradio_script_args.extend(["--lora-ckpt-path", arc_lora_path, checkpoint_path])
            else:
                gradio_script_args.extend(["--lora-ckpt-path", checkpoint_path])
            gradio_script_args.extend(["--model-half", "--title", title])
            if default_prompt_arg:
                gradio_script_args.extend(default_prompt_arg)
            if verbose:
                gradio_script_args.append("--verbose")
            ...
            launch_args, launch_env, _ = _build_launch_command(
                RUN_GRADIO_SCRIPT, gradio_script_args, env_vars
            )
...
def _discover_share_url_from_pipe(gradio_pid):
    ...
    Linux-only: reads /proc pipe inodes. Returns None on Windows.
    """
    if IS_WINDOWS:
        return None
    try:
...
def _discover_share_url_via_frpc(gradio_pid):
    ...
    Linux-only: reads /proc. Returns None on Windows.
    """
    if IS_WINDOWS:
        return None
    try:
...
def _detect_listening_port(pid):
    ...
    try:
        ...
        for conn in proc.net_connections(kind="tcp"):
            if conn.status == "LISTEN" and conn.laddr.port >= GRADIO_PORT_BASE:
...
def _recover_orphaned_gradios():
    ...
    recovered = 0
    # Scan for run_gradio.py processes using psutil
    import psutil
        ...
        while True:
            ...
            try:
                ...
                for run in self._registry.list_runs():
                    ...
                    env_vars = {
                        **backend_env,
                        "UNDERFIT_LOG_PATH": str(_app_path(log_path)),
                        "PYTHONUTF8": "1",
                    ...
                    # Parse restart_cmd (bash string) into args list
                    restart_script, restart_script_args = _split_restart_cmd(restart_cmd)

                    launch_args, launch_env, restart_cwd = _build_launch_command(
                        restart_script, restart_script_args, env_vars, cwd=demo_dir
                    )

                    # Open log in append mode (restarts append to existing log)
                ...
                # ── Queue drain: check for GPUs that became free ──
                all_runs = self._registry.list_runs()
                active_runs = [r for r in all_runs
                               if r.get("status") in ("training", "paused")]
                gpus_with_active = set(r.get("gpu") for r in active_runs if r.get("gpu") is not None)

                num_gpus = _get_gpu_count()
                for g in range(num_gpus):
                    # Reset gpu_parallel_count when GPU is fully free AND no queued runs
                    if g not in gpus_with_active:
                        still_queued = [r for r in all_runs
                                        if r.get("gpu") == g and r.get("status") == "queued"]
                        if not still_queued:
                            gpu_parallel_count.pop(g, None)
                    # Launch queued runs to fill up to the peak parallel count
                    # (runs for both busy GPUs filling slots, and free GPUs draining queue)
                    target = gpu_parallel_count.get(g, 0)
                    if target == 0:
                        continue
                    active_on_gpu = [r for r in all_runs
                                     if r.get("gpu") == g and r.get("status") == "training"]
                    slots_to_fill = target - len(active_on_gpu)
                    if slots_to_fill <= 0:
                        continue
                    queued = [r for r in all_runs
                              if r.get("gpu") == g and r.get("status") == "queued"]
                    for i in range(min(slots_to_fill, len(queued))):
                        self._launch_queued_run(queued[i])
            except Exception as e:
                print(f"[monitor] Error: {e}")

    def _launch_queued_run(self, run):
        """Launch a queued run: rebuild launch args from stored config and start the process."""
        run_id = run["id"]
        gpu = run.get("gpu")

        # Queued resume — simpler path: parse stored restart_cmd
        if run.get("queue_resume"):
            self._launch_queued_resume(run)
            return

        config_path = run.get("config_path", "")
        dataset_config_path = run.get("dataset_config_path", "")
        log_path = run.get("log_path", "")
        demo_dir = run.get("demo_source_dir", str(RUNS_DIR))
        save_dir = run.get("checkpoints_dir", str(RUNS_DIR))
        batch_size = run.get("batch_size", 8)
        lora_type = run.get("lora_type", "lora")
        rank = run.get("rank", 16)
        checkpoint_every = run.get("checkpoint_every", 1000)
        demo_every = run.get("demo_every", 1000)
        precision_flag = run.get("precision_flag", "16-mixed")
        num_workers = run.get("num_workers", 8)
        base_model = run.get("base_model", "sa3-medium")

        # Re-derive the base model checkpoint so the queued run loads the real
        # base weights (the immediate-launch path does this at its call site;
        # the run record doesn't store it). An empty path here means the
        # trainer skips base-weight loading entirely -> random init -> garbled
        # demos + a useless LoRA.
        mi = _get_model_info(base_model)
        base_ckpt = mi.get("base_ckpt", "") if mi else ""

        print(f"[queue] Launching queued run {run_id} on GPU {gpu}")

        train_script = BASE_DIR / "lora_train.py"
        train_script_args = [
            "--name", run_id,
            "--config-file", str(_app_path(BASE_DIR / "defaults.ini")),
            "--save-dir", str(_app_path(save_dir)),
            "--model-config", str(_app_path(config_path)),
            "--dataset-config", str(_app_path(dataset_config_path)),
            "--val-dataset-config", "",
            "--pretrained-ckpt-path", str(_app_path(base_ckpt)),
            ...
            "--max-steps", str(run.get("max_steps", 20000)),
            "--gradient-clip-val", "1.0",
            "--logger", "",
        ]

        # Also build restart_cmd for display/storage (backward compat)
        _q = shlex.quote
        restart_cmd = (
            ...
            f"     --model-config {_bash_quote(_app_path(config_path))}"
            f"     --dataset-config {_bash_quote(_app_path(dataset_config_path))}"
            f"     --val-dataset-config ''"
            f"     --pretrained-ckpt-path {_bash_quote(_app_path(base_ckpt))}"
            f"     --pretransform-ckpt-path ''"
            f"     --ckpt-path ''"
            f"     --num-nodes 1"
            f"     --num-workers {num_workers}"
            f"     --precision {precision_flag}"
            f"     --batch-size {batch_size}"
            f"     --checkpoint-every {checkpoint_every}"
            f"     --max-steps {run.get('max_steps', 20000)}"
        ...
        os.makedirs(demo_dir, exist_ok=True)
        launch_args, launch_env, launch_cwd = _build_launch_command(
            train_script, train_script_args, env_vars, cwd=demo_dir
        )
        ...
        self._registry.update_run(run_id,
                                  ...
                                  active=True)
        print(f"[queue] Launched queued run {run_id} on GPU {gpu} as PID {proc.pid}")

    def _launch_queued_resume(self, run):
        """Launch a queued resume: parse stored restart_cmd and start the process."""
        run_id = run["id"]
        gpu = run.get("gpu")
        restart_cmd = run.get("restart_cmd", "")
        log_path = run.get("log_path", "")
        ...
        print(f"[queue] Launching queued resume {run_id} on GPU {gpu}")

        # Parse restart_cmd (bash string) into args list
        resume_script, resume_script_args = _split_restart_cmd(restart_cmd)
        ...
        os.makedirs(demo_dir, exist_ok=True)
        launch_args, launch_env, resume_cwd = _build_launch_command(
            resume_script, resume_script_args, env_vars, cwd=demo_dir
        )
        ...
        self._registry.update_run(run_id,
                                   ...
                                   active=True,
                                   queue_resume=None)
        print(f"[queue] Launched queued resume {run_id} on GPU {gpu} as PID {proc.pid}")


training_monitor = TrainingMonitor(registry, gradio_manager)

# Track peak parallel training runs per GPU. When a GPU has no active runs
# and no queued runs targeting it, this resets to 0. The queue drain uses it
# as the target fill level for each GPU.
gpu_parallel_count = {}
...
# Cached CUDA→nvidia-smi index mapping (built on first _get_gpu_info call).
_cuda_to_nvidia: dict | None = None
    ...
    if model_config_path:
        ...
        if config_path.exists():
            try:
                ...
                # Timestep-ratio focus: null = feature disabled (default).
...
def _process_run_demos(run):
    ...
    # Copy individual demo files (demo_{i}_{step}.wav + .json sidecars)
    spec_futs = []
    for src in sorted(demo_source.glob("demo_*_*.wav")):
        if run_created_ts and src.stat().st_mtime < run_created_ts:
            continue  # Pre-dates this run
        m = re.match(r"demo_(\d+)_(\d+)\.wav", src.name)
        if not m:
            continue
        idx = int(m.group(1))
        step = int(m.group(2))
    ...
    # Named extra demos (e.g. demo_arc_00001000.wav)
    for src in sorted(demo_source.glob("demo_arc_*.wav")):
        if run_created_ts and src.stat().st_mtime < run_created_ts:
            continue
        m = re.match(r"demo_arc_(\d+)\.wav", src.name)
        if not m:
            continue
        step = int(m.group(1))
        step_dir = out_base / f"step_{step:08d}"
        step_dir.mkdir(parents=True, exist_ok=True)
        dest = step_dir / "demo_arc.wav"
...
class DashboardHandler(SimpleHTTPRequestHandler):
    def do_GET(self):
        ...
        if path == "/":
            ...
            if cached:
                file_info, total_files, files_with_tags, files_with_json, files_with_txt = cached
            ...
            if not cached:
                ...
                if input_dir and Path(input_dir).is_dir():
                    file_info, total_files, files_with_tags, files_with_json, files_with_txt, _csv_count = self._scan_audio_tags(
                        input_dir, sample_size=None, tag_sample_size=None)
                    _save_tag_cache(ds_id, file_info, total_files, files_with_tags, files_with_json, files_with_txt)
                else:
                    file_info, total_files, files_with_tags, files_with_json, files_with_txt = [], 0, 0, 0, 0
            self._json_response({
                "dataset": ds,
                "files": file_info,
                "total_files": total_files,
    ...
    def do_POST(self):
        ...
        if parsed.path == "/api/runs":
            ...
            length = int(self.headers.get("Content-Length", 0))
            body = json.loads(self.rfile.read(length)) if length else {}
            self._handle_queue_remove(body)
    ...
    def _handle_new_finetune(self, body):
        ...
        # Gradient-checkpointing toggle. Absent key (old client) -> leave the
        # template default (True) untouched below.
        ...
        try:
            ...
            _tr = body.get("timestep_ratio")
            if _tr is not None:
                cfg["training"].setdefault("timestep_sampler_options", {})["ratio"] = float(_tr)
            else:
                cfg["training"].get("timestep_sampler_options", {}).pop("ratio", None)
        ...
        # Map base_precision (dashboard UI) to Lightning-style --precision flag.
        # "" (fp32) → 32-true, "fp16" → 16-mixed, "bf16" → bf16-mixed
        if base_precision == "bf16":
            precision_flag = "bf16-mixed"
        elif base_precision == "fp16":
            precision_flag = "16-mixed"
        else:
            precision_flag = "32-true"

        # Build training args as a proper list (cross-platform)
        train_script = BASE_DIR / "lora_train.py"
        train_script_args = [
            "--name", run_id,
            "--config-file", str(_app_path(BASE_DIR / "defaults.ini")),
            "--save-dir", str(_app_path(save_dir)),
            "--model-config", str(_app_path(run_config_path)),
            "--dataset-config", str(_app_path(dataset_config)),
            "--val-dataset-config", "",
            "--pretrained-ckpt-path", str(_app_path(mi["base_ckpt"])),
            "--pretransform-ckpt-path", "",
            "--ckpt-path", "",
            "--num-nodes", "1",
            "--num-workers", "8",
        ...
        # Also build restart_cmd string for display/storage (backward compat)
        _q = shlex.quote
        restart_cmd = (
            f"python {_bash_quote(_app_path(BASE_DIR / 'lora_train.py'))}"
            f"     --name {_q(run_id)}"
        ...
        run = {
            ...
            "max_steps": max_steps,
            "active": not queue_flag,
            "status": "queued" if queue_flag else "training",
            "step_offset": 0,
            "created_at": datetime.now(timezone.utc).isoformat(),
            "pid": proc.pid if not queue_flag else None,
            "gpu": gpu,
            "restart_cmd": restart_cmd if not queue_flag else None,
        ...
        # Track peak parallel training runs per GPU for queue drain logic.
        # Only manual (non-queued) launches update the count.
        if not queue_flag:
            active_on_gpu = [r for r in registry.list_runs()
                             if r.get("gpu") == gpu and r.get("status") == "training"]
            if active_on_gpu:
                gpu_parallel_count[gpu] = gpu_parallel_count.get(gpu, 0) + 1
            else:
                gpu_parallel_count[gpu] = 1
        registry.add_run(run)
        (AUDIO_DIR / "runs" / run_id).mkdir(parents=True, exist_ok=True)
        if queue_flag:
            print(f"[control] Queued finetune {run_id} on GPU {gpu}, max_steps {max_steps}")
            self._json_response({"ok": True, "id": run_id, "display_name": name, "status": "queued"}, status=201)
        else:
    ...
    def _handle_resume(self, run_id, body):
        ...
        try:
            ...
            _tr = body.get("timestep_ratio")
            if _tr is not None:
                cfg.setdefault("training", {}).setdefault("timestep_sampler_options", {})["ratio"] = float(_tr)
            else:
                cfg.get("training", {}).get("timestep_sampler_options", {}).pop("ratio", None)
        ...
        env_vars = {
            **backend_env,
            "UNDERFIT_LOG_PATH": str(_app_path(new_log)),
            "PYTHONUTF8": "1",
            "PYTHONIOENCODING": "utf-8",
            "PYTHONUNBUFFERED": "1",
            "MPLBACKEND": "Agg",
        ...
        # Parse new_cmd (bash string) into args list
        resume_script, resume_script_args = _split_restart_cmd(new_cmd)

        launch_args, launch_env, resume_cwd = _build_launch_command(
            resume_script, resume_script_args, env_vars, cwd=demo_dir
        )

        # Track peak parallel count for queue drain logic
        active_on_gpu = [r for r in registry.list_runs()
                         if r.get("gpu") == gpu and r.get("status") == "training"]
        if active_on_gpu:
            gpu_parallel_count[gpu] = gpu_parallel_count.get(gpu, 0) + 1
        ...
        if queue_flag:
            # Queue the resume — store config paths for later launch
            registry.update_run(run_id,
                ...
                restart_cmd=new_cmd,
                gpu=gpu,
                config_path=str(resume_config_path),
                dataset_config_path=str(resume_ds_path) if resume_ds_path else None,
                queue_resume=True,
                queued_at=datetime.now(timezone.utc).isoformat(),
            )
            with _hyperparams_cache_lock:
                _hyperparams_cache.pop(run_id, None)
            print(f"[control] Queued resume {run_id} on GPU {gpu}, steps {effective_offset} -> {new_max_steps}")
            self._json_response({"ok": True, "status": "queued", "new_max_steps": new_max_steps})
            return

        try:
            log_f = open(str(new_log), "w")
            proc = _popen_managed(
                launch_args,
                stdout=log_f,
                stderr=log_f,
    ...
    def _get_gpu_info(self):
        ...
        try:
            out = subprocess.check_output(
                ["nvidia-smi", "--query-gpu=index,memory.used,memory.total,memory.free,utilization.gpu,name",
                 "--format=csv,noheader,nounits"],
                timeout=15, stderr=subprocess.DEVNULL,
            ).decode().strip()
            for line in out.split("\n"):
        ...
        resp = {"gpus": gpus, "gradio_estimate": _gradio_vram, "arc_info": arc_info, "cuda_to_nvidia": cuda_to_nvidia}
    ...
    def _get_gpu_processes(self, gpu_idx):
        ...
        # Convert CUDA index → nvidia-smi index using the cached mapping
        nvidia_idx = _get_cuda_to_nvidia().get(gpu_idx, gpu_idx)
        # Query nvidia-smi for compute processes
        try:
            out = subprocess.check_output(
                ["nvidia-smi", "-i", str(nvidia_idx),
        ...
        for r in registry.list_runs():
            ...
            if pid:
                ...
                for cpid in _get_children_pids(pid):
        ...
        for line in out.split("\n"):
            ...
            # Read cmdline (cross-platform via psutil)
            cmdline = _get_process_cmdline(pid)

            # Skip processes owned by other users
            proc_user = _get_process_username(pid)
            if proc_user and proc_user != my_user:
                short = os.path.basename(cmdline.split()[0]) if cmdline else f"pid-{pid}"
                processes.append({
    ...
    def _handle_adopt(self, body):
        ...
        cmdline = _get_process_cmdline(pid)
        if not cmdline:
            self._json_response({"error": f"cannot read cmdline for PID {pid}"}, status=400)
            return
        ...
        # Find CWD (demo dir) — cross-platform via psutil
        try:
            import psutil
            cwd = str(psutil.Process(pid).cwd())
        except Exception:
        ...
        # Reconstruct restart_cmd from the live process's raw argv list.
        # Use psutil's argv list (not the space-joined string) so empty-string
        # args and paths with spaces survive, and normalize backslashes to
        # forward slashes so shlex.split round-trips on Windows. The leading
        ...
        if raw_argv:
            ...
            def _fmt(tok):
                p = _bash_path(tok)
                # Readability: store home-based paths as ~/... (expanded back
                # to an absolute path by _split_restart_cmd at restart time).
                if p == home_bash:
                    return shlex.quote("~")
                if p.startswith(home_bash + "/"):
                    return shlex.quote("~" + p[len(home_bash):])
                return shlex.quote(p)
            restart_cmd = "python " + " ".join(_fmt(a) for a in raw_argv)
        else:
            # Fallback: best-effort from the space-joined cmdline (legacy)
    ...
    def _handle_queue_remove(self, body):
        """Remove a queued run from the training queue."""
        run_id = body.get("run_id")
        if not run_id:
        ...
        if not run or run.get("status") != "queued":
            self._json_response({"error": "run not found or not queued"}, status=404)
            return
        registry.remove_run(run_id)
        print(f"[queue] Removed queued run {run_id}")
        self._json_response({"ok": True})

    def _handle_save_checkpoint(self, body):
        """Trigger a manual checkpoint save on a running training process.

        Linux: send SIGUSR1 to the python child process.
        Windows: create a flag file that the training loop polls every 10 steps.
        """
        run_id = body.get("run_id")
        if not run_id:
            self._json_response({"error": "run_id required"}, status=400)
        ...
        if sigusr1 is not None:
            child_pids = _get_children_pids(pid)
            target_pid = pid
            for cpid in child_pids:
                try:
                    import psutil
                    comm = psutil.Process(cpid).name()
        ...
        # --- Windows: file-based flag ---
        save_dir = run.get("checkpoints_dir", str(RUNS_DIR))
        flag_path = os.path.join(save_dir, run_id + "_save_ckpt.flag")
        try:
            with open(flag_path, "w") as f:
                f.write(str(time.time()))
            print(f"[save_checkpoint] Created flag file {flag_path} (run {run_id}, PID {pid})")
            self._json_response({"ok": True, "flag_path": flag_path})
        except Exception as e:
            self._json_response({"error": f"Failed to create flag file: {e}"}, status=500)
    ...
    def _scan_audio_tags(dir_path, sample_size=50, tag_sample_size=50,
        ...
        # Collect ALL .txt files for cross-directory sidecar matching
        txt_files = {}  # stem -> list of absolute Paths
            ...
            for fn in filenames:
                ...
                if _is_audio_file(fp):
                    ...
                    if low.endswith(".json"):
                        ...
                        stem = Path(fn).stem
                        txt_files.setdefault(stem, []).append(fp)
        ...
        if tag_limit > 0:
            ...
            with ThreadPoolExecutor(max_workers=16) as pool:
                futures = {pool.submit(_read_audio_tags, fp, json_files, txt_files): i
        ...
        for i, fpath in enumerate(returned):
            ...
            if has_json:
                files_with_json += 1
            has_txt = _find_txt_sidecar(fpath, txt_files) is not None
            if has_txt:
                files_with_txt += 1
            ...
            has_embedded_tags = bool(tags) and not has_json and not has_txt
        ...
        return file_info, total, files_with_tags, files_with_json, files_with_txt, csv_count
    ...
    def _handle_datasets_scan(self, body):
        ...
        file_info, total_files, files_with_tags, files_with_json, files_with_txt, csv_count = \
    ...
    def _handle_datasets_encode(self, body):
        ...
        file_info, _, files_with_tags, files_with_json, files_with_txt, _csv_count = self._scan_audio_tags(
        ...
        _save_tag_cache(ds_id, file_info, num_files, files_with_tags, files_with_json, files_with_txt)
    ...
    def _get_demos(self, run_id=None):
        ...
        for step_dir in dir_entries:
            ...
            if not step_dir.is_dir():
                continue
            clips = []
            for wav in sorted(step_dir.glob("demo_*.wav")):
                dm = re.match(r"demo_(\d+)\.wav", wav.name)
                if dm:
                    clip = {
                        "index": int(dm.group(1)),
                        "url": f"/audio/runs/{run_id}/{step_dir.name}/{wav.name}",
                        "size_mb": round(wav.stat().st_size / 1e6, 1),
                    }
                    # Add spectrogram URL if JPG exists
                    jpg_path = wav.with_suffix(".jpg")
                    if jpg_path.exists():
            ...
            if arc_path.exists():
                arc_clip = {
                    "index": "arc",
                    "url": f"/audio/runs/{run_id}/{step_dir.name}/demo_arc.wav",
