...
# ---------------------------------------------------------------------------
# Audio loading — torchaudio 2.11+ requires torchcodec (FFmpeg DLLs).
# Fall back to soundfile for all supported formats so the encoder works
# without any external binary dependencies.
# ---------------------------------------------------------------------------

_LOADED_FMT = {"wav", "flac", "mp3", "ogg", "aiff", "wave", "voc", "svx", "nist", "au", "mat4", "mat5", "pvf", "xi", "htk", "sds", "avr", "w64", "raw", "pafr", "rf64", "irCAM", "mpc2k", "vocal", "m4a", "aac", "wma", "opus", "amr", "caf"}
try:
    _SUPPORTED_FMT = {f.lower() for f in sf.available_formats() if f.lower() in _LOADED_FMT}
except Exception:
    _SUPPORTED_FMT = {"wav", "flac", "mp3", "ogg", "aiff"}

def _load_audio_soundfile(path, frame_offset=0, num_frames=-1, normalize=True,
                          channels_first=True, format=None, buffer_size=4096, backend=None):
    """Load audio via soundfile, return (waveform_tensor, sample_rate)."""
    data, sr = sf.read(str(path), start=frame_offset, stop=num_frames,
...
_torchcodec_unavailable = False
try:
    import torchcodec  # noqa: F401
except (ImportError, OSError, RuntimeError):
    _torchcodec_unavailable = True
if _torchcodec_unavailable:
...
def main():
    ...
    if args.batch_size > 0:
        ...
        target_mb = gpu_mem_mb * 0.80  # use up to 80% of VRAM
        model_mb = 300
