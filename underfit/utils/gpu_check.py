...
def check_attention_backends() -> dict:
    ...
    status_icon = "OK" if paths["flash_attn"] else "SLOW"
    print(f"  [{status_icon}] Attention backend: {primary}", flush=True)
    print(f"         Available: {', '.join(available)}", flush=True)
    if not paths["flash_attn"]:
        print(f"         flash-attn not installed — will use SDPA fallback (slower on Ampere+ with batch>1)", flush=True)
    return paths
