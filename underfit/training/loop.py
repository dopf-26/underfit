...
def run_training(args, backend):
    ...
    # --- Mixed precision setup (resolved early so model loading uses the right dtype) ---
    ...
    # File-based save flag (Windows fallback). Checked every 10 steps.
    # Path: <save_dir>/<name>_save_ckpt.flag
    _save_flag_path = None
    if args.save_dir and args.name:
        _save_flag_path = os.path.join(args.save_dir, args.name + "_save_ckpt.flag")
    ...
    # Gradient checkpointing: on by default (SA3 transformer default) to bound
    # activation VRAM. Disable to trade VRAM for speed — longer sequences and
    # bigger models will use noticeably more VRAM and may OOM. Plumbed through to
    # ContinuousTransformer.forward(use_checkpointing=...) via the kwargs chain.
    use_checkpointing = bool(training_config.get("use_checkpointing", True))
    if not use_checkpointing:
        print("[startup] Gradient checkpointing DISABLED — expect higher activation VRAM "
              "(long sequences / large models may OOM)", flush=True)
            ...
            for batch_idx, batch in enumerate(pbar):
                ...
                with amp_ctx:
                    ...
                    output = model(noised, t, cond=conditioning,
                                  ...
                                  use_checkpointing=use_checkpointing, **extra_args)
                ...
                # --- File-based save flag check (Windows IPC). Polls every 10 steps.
                if _save_flag_path is not None and global_step % 10 == 0:
                    try:
                        if os.path.exists(_save_flag_path):
                            manual_save_requested[0] = True
                            os.remove(_save_flag_path)
                            print(f"\n[save flag] Checkpoint save requested via {_save_flag_path}", flush=True)
                    except Exception:
                        pass
