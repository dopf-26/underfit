...
the dashboard already ingests: demo_<i>_<step:08d>.wav + .json sidecar in
...
def run_demo_step(model, backend, demo_config, step, sample_size, sample_rate, device, model_config=None):
    ...
    Writes demo_<i>_<step:08d>.wav + .json into cwd. Skips entries that
    already exist on disk (idempotent for resumes).
    """
    demo_cond = demo_config.get("demo_cond") or []
    if not demo_cond:
        return
    if os.path.exists(f"demo_0_{step:08d}.wav"):
