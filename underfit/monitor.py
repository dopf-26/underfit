...
def launch_dashboard_subprocess(*, port: int = 8787,
    ...
    if kill_existing:
        # Cross-platform: kill any process listening on the given TCP port
        try:
            import psutil
...
def debug_info(state_dir: str = "/content/drive/MyDrive/underfit-state",
    ...
    try:
        ...
        for proc in psutil.process_iter(["pid", "cmdline"]):
            ...
            if re.search(r"python\S* .*dashboard/server\.py", cmdline_str):
                matched.append(f"  {proc.info['pid']}  {cmdline_str}")
        if matched:
            print("\n".join(matched))
        else:
            print("  (no dashboard/server.py process found)")
    except Exception as e:
