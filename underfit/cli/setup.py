...
class Backend:
    ...
    deps: tuple[str, ...] = ()    # explicit deps to install alongside (avoids backend torch pins)
...
BACKENDS: list[Backend] = [
    Backend(
        ...
        deps=(
            # Base SA3 deps (torch/torchaudio omitted — managed by underfit pyproject.toml)
            "einops>=0.8.2",
            "einops-exts>=0.0.4",
            "packaging>=26.0",
            "transformers>=5.8.0",
...
def _arrow_select(title: str, lines_per_option: list[list[str]], default_idx: int) -> int:
    ...
    or on Windows (termios/tty unavailable).
    """
    n = len(lines_per_option)

    def _numeric_fallback():
        """Non-interactive / cross-platform fallback: print menu + typed digit."""
        print(title)
    ...
    print(title)
    print("(up/down to move, Enter to select, q to quit)")
    print()
    _render(first=True)
    try:
        tty.setcbreak(fd)
...
def _editable_install(path: Path, extras: tuple[str, ...] = (), backend_deps: tuple[str, ...] = ()) -> int:
    """Install a backend as editable package.

    When `backend_deps` is provided, installs the editable package with
    --no-deps (so the backend's own torch pins can't override underfit's),
    then resolves the explicit deps + extras separately via uv pip install.

    When `backend_deps` is empty, falls back to the traditional
    `pip install -e <path>[extras]` path (used by sat)."""
    if backend_deps:
        # --no-deps editable install — registers the package without
        # pulling in the backend's own torch/torchaudio pins.
        cmd = [*_install_command(), "--no-deps", "-e", str(path)]
        print(f"\n→ {' '.join(cmd)}\n", flush=True)
        rc = subprocess.run(cmd).returncode
        if rc != 0:
            return rc

        # Resolve backend deps separately. uv sees only the
        # explicit requirements here and can't downgrade torch.
        # (backend.deps already includes all needed packages from extras.)
        cmd = [*_install_command()] + list(backend_deps)
        print(f"\n→ {' '.join(cmd)}\n", flush=True)
        rc = subprocess.run(cmd).returncode
    else:
...
def _download_repo(repo_id: str, target_dir: Path) -> Path | None:
    ...
    try:
        ...
        # Windows without developer mode: symlink fails, fall back to copy
        print(f"  symlink failed ({link_err}), copying directory...")
        import shutil as _shutil
        _shutil.copytree(snapshot_path, target_dir)
        print(f"  ready at {target_dir} (copy of {snapshot_path})")
...
def _provision_backend(backend: Backend, args) -> bool:
    ...
    if getattr(args, "backend_path", None):
        ...
        if args.no_install:
            ...
            return True
        rc = _editable_install(path, backend.extras, backend.deps)
        if rc != 0:
    ...
    rc = _editable_install(checkout_path, backend.extras, backend.deps)
    if rc != 0:
